...
const Invoicing: React.FC<{
  ...
  const [filteredProducts, setFilteredProducts] = useState<Record<number, Product[]>>({});
  ...
  const updateItem = (index: number, field: keyof InvoiceItemDraft | string, value: any) => {
    ...
    if (field === "quantity") cur.quantity = Math.max(0, Math.floor(safeNum(value)));
  ...
  /* ---------- product select: unit price ALWAYS from products.price (rule B) ---------- */
  const handleProductSelect = (index: number, product_id: string) => {
    const prod = products.find(p => p.id === product_id);
    if (!prod) return;
    const price = Number(prod.price ?? 0); // RULE B: use product.price only
    const list = [...items];
    const cur = { ...list[index] } as any;
    ...
    // Recompute discount amount if discount_percent exists
    ...
    // reset per-row search state & close dropdown
    setProductSearch(prev => ({ ...prev, [index]: "" }));
    setFilteredProducts(prev => ({ ...prev, [index]: products }));
    setOpenProductDropdown(null);
  };

  /* ---------- variant select: only update description & stock, DO NOT change price ---------- */
  const handleVariantSelect = (index: number, variant_id: string) => {
    ...
    if (variant) {
      ...
      cur.description = `${base} (${variant.color ?? ""} ${variant.size ?? ""})`.trim();
    }
    // do NOT change unit_price - per rule B
    // but recalc line total just in case quantity/discount changed
    const qty = Number(cur.quantity || 0);
    const unit = Number(cur.unit_price || 0);
    const disc = Number(cur.discount_amount || 0);
  ...
  /* ---------- payment helpers (kept here for coherence, further logic in final section) ---------- */
  ...
  /* ---------- small helper to write history (used in save flow) ---------- */
  ...
  const totalDiscount = useMemo(
    () =>
      items.reduce((s, it) => {
        ...
        const disc = (it.discount_amount && it.discount_amount > 0)
          ? Number(it.discount_amount)
          : (it.discount_percent && it.discount_percent > 0 ? unit * (it.discount_percent / 100) : 0);
        return s + (disc * qty);
      }, 0),
    [items]
  );
  ...
  const handleSaveInvoice = async () => {
    ...
    try {
      ...
      const subtotalLocal = items.reduce((s, it) => s + (Number(it.unit_price || 0) * Number(it.quantity || 0)), 0);
      ...
      const taxable = Math.max(0, subtotalLocal - totalDiscountLocal);

      // tax splits
      let cgstLocal = 0, sgstLocal = 0, igstLocal = 0;
      if (form.taxType === "CGST_SGST") {
        const halfPercent = (form.taxPercent || 0) / 2;
        cgstLocal = Number(((taxable * halfPercent) / 100).toFixed(2));
        sgstLocal = Number(((taxable * halfPercent) / 100).toFixed(2));
      } else if (form.taxType === "IGST") {
        igstLocal = Number(((taxable * (form.taxPercent || 0)) / 100).toFixed(2));
      }

      const grandTotalLocal = Number((taxable + cgstLocal + sgstLocal + igstLocal).toFixed(2));
      ...
      /* ---------- 3) DEDUCT STOCK USING RPC ---------- */
      for (const it of items) {
        if (it.type !== "product" || !it.product_id) continue;
        if (it.variant_id) {
          const { error: rpcErr } = await supabase.rpc("deduct_variant_stock", {
            p_variant_id: it.variant_id,
            p_quantity: Math.floor(Number(it.quantity)),
          });
      ...
      try {
        ...
        console.warn("Failed to record payment/daily income", err);
      }
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-col md:flex-row gap-2">
            <div className="flex-1 relative">
              <input
                className="border p-3 rounded w-full text-sm focus:outline-none focus:ring"
              ...
              {showCustomerList && customerResults.length > 0 && (
                <div className="absolute z-50 bg-white border rounded mt-1 w-full max-h-48 overflow-auto shadow">
                  {customerResults.map(c => (
                    <div key={c.id} className="px-3 py-3 hover:bg-gray-100 cursor-pointer" onClick={() => handleSelectCustomer(c)}>
          ...
          <input className="border p-3 rounded w-full text-sm" placeholder="Customer Name" value={form.customer_name} onChange={(e) => { const name = e.target.value; setForm(prev => ({ ...prev, customer_name: name })); try { onCustomerNameChange(name); } catch (_) {} }} />
          <div className="flex gap-2">
            <input className="border p-3 rounded w-full text-sm" placeholder="Customer Phone" value={form.customer_phone} onChange={(e) => setForm(prev => ({ ...prev, customer_phone: e.target.value }))} />
            <input className="border p-3 rounded w-64 text-sm" placeholder="Customer State (for GST)" value={form.customer_state} onChange={(e) => setForm(prev => ({ ...prev, customer_state: e.target.value }))} />
          </div>
          <input className="border p-3 rounded w-full text-sm" placeholder="Reference By (salesperson / referred by)" value={form.reference_by} onChange={(e) => setForm(prev => ({ ...prev, reference_by: e.target.value }))} />
        </CardContent>
      </Card>
      ...
      <Card className="shadow-sm border">
        ...
        <CardContent className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm text-gray-600">Rows: <span className="font-semibold">{items.length}</span></div>
          </div>
          ...
          <div className="border rounded overflow-hidden">
            ...
            <div className="max-h-[420px] overflow-auto">
              {items.map((it, idx) => {
                ...
                return (
                  <div key={idx} className="px-3 py-3 border-b flex items-start gap-3">
                    <div className="w-8 text-sm text-gray-700">{idx + 1}</div>
                    ...
                    <div className="flex-1">
                      {it.type === "product" ? (
                        <>
                              ...
                              {productVariants.map(v => {
                                ...
                                return (
                                  <option key={v.id} value={v.id} disabled={stock <= 0}>
                                    {v.color ?? ""} {v.size ?? ""} — {stock > 0 ? stock : "OUT"}
                                  </option>
                                );
                              })}
                            </select>
                          ...
                          <input className="border rounded p-2 mt-2 w-full text-sm" placeholder="Item description" value={it.description} onChange={(e) => updateItem(idx, "description", e.target.value)} />
                          <div className="mt-2 flex items-center gap-3 text-xs text-gray-500">
                            {it.product_id && <div>Stock: <span className={productStock > 0 ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}>{productStock}</span></div>}
                            {it.variant_id && <div>Variant stock: <span className={variantStock > 0 ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}>{variantStock}</span></div>}
                            <div className="ml-auto">HSN: <input className="ml-2 border rounded p-1 text-xs w-28" value={it.hsn_code || ""} onChange={(e) => updateItem(idx, "hsn_code", e.target.value)} /></div>
                          </div>
                        </>
                      ) : (
                        <input className="border rounded p-3 w-full text-sm" placeholder="Manual item description" value={it.description} onChange={(e) => updateItem(idx, "description", e.target.value)} />
                      )}
                    </div>

                    <div className="w-48 text-right">
                      <div className="text-xs text-gray-500">Unit</div>
                      <input type="number" className="border rounded p-2 text-sm text-right w-full mt-1" value={it.unit_price} onChange={(e) => updateItem(idx, "unit_price", e.target.value)} />
                      <div className="text-xs text-gray-400 mt-1">Always from product.price</div>
                    </div>

                    <div className="w-28 text-right">
                      <div className="text-xs text-gray-500">Qty</div>
                      <input type="number" min={0} className="border rounded p-2 text-sm text-right w-full mt-1" value={it.quantity} onChange={(e) => updateItem(idx, "quantity", e.target.value)} />
                    </div>

                    <div className="w-28 text-right">
                      <div className="text-xs text-gray-500">Disc ₹</div>
                      <input type="number" min={0} step="0.01" className="border rounded p-2 text-sm text-right w-full mt-1" value={it.discount_amount || 0} onChange={(e) => updateItem(idx, "discount_amount", e.target.value)} />
                      <div className="text-xs text-gray-400 mt-1">or % <input className="ml-2 w-12 border rounded p-1 text-xs" value={it.discount_percent || 0} onChange={(e) => updateItem(idx, "discount_percent", e.target.value)} /></div>
                    </div>

                    <div className="w-32 text-right">
                      <div className="text-xs text-gray-500">Line</div>
                      <div className="text-lg font-semibold mt-1">₹{Number(it.total || 0).toFixed(2)}</div>
                    </div>

                    <div className="w-12 text-center">
                      <button className="text-red-500 text-sm" onClick={() => removeItem(idx)}>✕</button>
                    </div>
                  </div>
                );
              })}
              ...
              {items.length === 0 && (
                <div className="p-6 text-center text-gray-500">No items yet. Add a product line or a manual item.</div>
              )}
            </div>
          </div>
        </CardContent>
      ...
      <Card className="shadow-sm border">
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-col md:flex-row gap-3 md:items-center">
            <select className="border rounded p-3 w-full md:w-48" value={form.payment_status} onChange={(e) => handlePaymentStatusChange(e.target.value as PaymentStatus)}>
              <option value="unpaid">Unpaid</option>
              <option value="partial">Partial Paid</option>
              <option value="paid">Paid</option>
            </select>

            {form.payment_status === "partial" && (<input type="number" className="border rounded p-3 w-full md:w-48" placeholder="Paid amount" value={form.paid_amount} onChange={(e) => handlePaidAmountChange(e.target.value)} />)}
          </div>
      ...
      {showCustomerModal && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
          <Card className="w-full max-w-md shadow-lg border bg-white">
            ...
            <CardContent className="space-y-3">
              <input className="border rounded p-3 w-full text-sm" placeholder="Customer Name" value={newCustomer.name} onChange={(e) => setNewCustomer(prev => ({ ...prev, name: e.target.value }))} />
              <input className="border rounded p-3 w-full text-sm" placeholder="Phone" value={newCustomer.phone} onChange={(e) => setNewCustomer(prev => ({ ...prev, phone: e.target.value }))} />
              <input className="border rounded p-3 w-full text-sm" placeholder="State" value={newCustomer.state} onChange={(e) => setNewCustomer(prev => ({ ...prev, state: e.target.value }))} />
              <input className="border rounded p-3 w-full text-sm" placeholder="Address" value={newCustomer.address} onChange={(e) => setNewCustomer(prev => ({ ...prev, address: e.target.value }))} />
