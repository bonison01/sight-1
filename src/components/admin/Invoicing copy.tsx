                  ...
                  <div key={idx} className="px-3 py-3 border-b flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      {it.type === "product" ? (
                        <>
                          <div className="flex gap-2">
                            <select className="border rounded p-2 text-sm w-1/2" value={it.product_id ?? ""} onChange={(e) => handleProductSelect(idx, e.target.value)}>
                              <option value="">Select product</option>
                              {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                            </select>

                            <select className="border rounded p-2 text-sm w-1/2" value={it.variant_id ?? ""} disabled={!it.product_id} onChange={(e) => handleVariantSelect(idx, e.target.value)}>
